#!/usr/bin/env python3
"""
Gerador V13 - Auto-detecta TODOS os tipos usados no bytecode.
Correcoes:
1. Le class pool do JAR e extrai nomes de classe referenciados
2. Gera typedef void* pra TODOS os tipos desconhecidos
3. Palavras-chave C (completa) sanitizadas
4. Inner classes ($) sanitizadas
5. Classe/metodo chamado "main" -> main_cls
6. Assinatura uniforme em todos metodos
"""
import sys, json, os, zipfile, io, re
from jawa.cf import ClassFile

# Palavras-chave C COMPLETAS
C_KEYWORDS = {
    ...
    "NULL","void_x","this","new","delete","try","catch","throw","class","public",
    "private","protected","virtual","template","typename","namespace","using",
    "operator","friend","mutable","explicit","export","asm","wchar_t"
}
...
def extrair_tipos_do_jar(jar_path):
    """Le TODAS as referencias de classe do JAR e retorna nomes simples."""
    tipos = set()
...
def gerar(estrutura, jar_path):
    ...
    # Auto-detecta tipos do JAR
    tipos_detectados = extrair_tipos_do_jar(jar_path)
    
    L = []
    L.append(f"// {nome} - main.c gerado por V13")
    ...
    # TIPOS BASE + AUTO-DETECTADOS
    L.append("// Tipos J2ME (auto-detectados)")
    tipos_base = ["Image","Graphics","DirectGraphics","Font","String","StringBuffer",
    ...
    todos_tipos = sorted(set(tipos_base) | set(tipos_detectados))
    for t in todos_tipos:
        t_san = san(t)
        # Evita redeclarar tipos base com nome igual
        L.append(f"typedef void* {t_san};")
    L.append("")
    ...
    # Metodos (com tratamento de colisao)
    L.append("// Prototipos")
    z = zipfile.ZipFile(jar_path)
    metodos = []
    contadores = {}
...
if __name__ == "__main__":
    ...
    with open("/data/data/com.termux/files/home/main_v13.c", "w") as f:
        f.write(codigo)
    print(f"Gerado: {len(codigo.splitlines())} linhas")
    print(f"Tipos auto-detectados: {n_tipos}")
