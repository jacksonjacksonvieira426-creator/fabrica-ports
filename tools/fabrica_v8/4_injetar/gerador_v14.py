#!/usr/bin/env python3
"""
V14 - Corrige o que faltou no V13:
1. Sanitiza tipo_c dos campos (Blackjack$c -> Blackjack_c)
2. Dedup de typedefs (MemoryCanvas, MIDlet1)
3. +tipos J2ME (TextBox, Alert, List, ImageItem, Spacer, Gauge, DateField, ChoiceGroup)
4. Colisao classe/metodo mais agressiva
"""
...
TIPOS_BASE = [
    ...
    "Calendar","Date","TimeZone","Locale","Character","StringTokenizer",
]
...
def gerar(estrutura, jar_path):
    ...
    L = []
    L.append(f"// {nome} - main.c gerado por V14")
    ...
    # TIPOS unicos (dedup automatico)
    todos_tipos = sorted(set([san(t) for t in TIPOS_BASE + tipos_detectados]))
    L.append("// Tipos J2ME (auto-detectados, dedup)")
    ...
    # FORWARD typedefs (dedup)
    L.append("// Forward typedefs")
    classes_san = {}
    nomes_classes_usados = set()
    for c in estrutura["classes"]:
        n = san(c["nome"])
        if n == "main":
            n = "main_cls"
        # Se ja existe (colisao), adiciona sufixo
        base = n
        contador = 1
        while n in nomes_classes_usados:
            contador += 1
            n = f"{base}_{contador}"
        nomes_classes_usados.add(n)
    ...
    for c in estrutura["classes"]:
        ...
        for campo in c["campos"]:
            tipo_c = campo["tipo_c"]
            # Sanitiza tipo_c: remove $, sanitiza
            tipo_c = tipo_c.replace("$", "_")
            # Se for ponteiro de classe, mantem; senao vira void*
    ...
    for c in estrutura["classes"]:
        ...
        for m in list(cf.methods):
            ...
            original = nome_fn
            contador = 1
            while nome_fn in nomes_fn_usados:
                contador += 1
...
if __name__ == "__main__":
    ...
    with open("/data/data/com.termux/files/home/main_v14.c", "w") as f:
        f.write(codigo)
